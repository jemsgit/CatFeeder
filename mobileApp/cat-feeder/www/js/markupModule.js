(function () {
    ...
    var privateTabs = [1, 2];
    var devicesTempate = _.template('<div class="device-item" data-id="<%- id %>" data-name="<%- name %>">'+
        '<%- name %><span class="loader button-loader"></span></div>');
    ...
    function getAttributeValue(e, attrVal){
        return $(e.target).attr(attrVal);
    }

    function setElementVisibility(id, flag) {
    ...
    app.modules.markup = {
        createUI: function () {
            ...
            setElementVisibility('#dis', false);
        },
        enableTabs: function (flag) {
            var list = flag ? [] : privateTabs;
            $("#tabs").tabs('option', 'disabled', list);
        },
        showSpinner: function (id, flag) {
            setElementVisibility(id, flag);
        },
        showCurrentDevice: function (device) {
            this.showDevices(false);
            this.showDeviceInfo(device);
            this.enableTabs(true);
            setElementVisibility('#dis', true);
            setElementVisibility('#refresh', false);
        },
        ...
        showDevices: function (list) {
            $('#device-info').html('');
            setElementVisibility('#dis', false);
            setElementVisibility('#refresh', true);
            var $markup = $('<div></div>');
            if (list) {
        ...
        addMessage: function (type, message) {
            var mes = '';
            switch (type) {
                case app.constants.incomeMessageType:
                    mes = incomeMessage({ message: message });
                    break;
                case app.constants.outcomeMessageType:
        ...
        attachEvents: function (callbacks) {
            ...
            $('#devices').on('click', '.device-item', function (e) {
                if (callbacks && callbacks.selectDevice) {
                    var id = getAttributeValue(e, 'data-id');
                    var name = getAttributeValue(e, 'data-name');
